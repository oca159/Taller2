<script>
    ...
    $(document).on('click','#entregar',function(e){
        if( $(this).hasClass('disabled') )
            ...
            var url  = '{!! url("json_libros_prestamo/' + id +'")!!}';
            $.get(url, function(result){
                var libros = result.length;
                var dias = $('#'+id+' .dias').data('id');
                multa = libros * dias;

                $('#costo_multa').html('$ '+ multa +'.00');
                $('#modal_entregar').openModal();
    ...
    $(document).on('click','#enviar_multa',function(e){
        if( $(this).hasClass('disabled') )
            e.preventDefault();
        else{
            var id_multa = $('#'+id).data('id');
            var url  = '{!! url("entregar_retraso/' + id +'/'+ id_multa +'/'+ multa +'")!!}';
            $.get(url, function(result){
                alert(result);
                window.location.replace('{!! url("/admin/retrasos") !!}');
            }).fail(function(){
                alert('Error');
            });
        }
